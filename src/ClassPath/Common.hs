...
ppMethodKey (MethodKey name params ret) = text name <> (parens . commas . map ppType) params <> maybe "void" ppType ret
  where
...
data Instruction
  ...
  | Invokeinterface ClassName
                    MethodKey -- ^ Invoke interface method
  | Invokespecial Type
                  MethodKey -- ^ Invoke instance method; special handling for
                                        -- superclass, private, and instance initialization
                                        -- method invocations
  | Invokestatic ClassName
                 MethodKey -- ^ Invoke a class (@static@) method
  | Invokevirtual Type
                  MethodKey -- ^ Invoke instance method; dispatch based on class
  | Ior -- ^ Boolean OR @int@
  | Irem -- ^ Remainder @int@
  ...
  | Multianewarray Type
                   Word8 -- ^ Create new multidimensional array (element type, # of dimensions)
...
safeNextPcPrim istrm pc
  | pc <= snd (bounds istrm) = Just $ nextPcPrim istrm pc
  | otherwise = Nothing
