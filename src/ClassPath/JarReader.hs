...
import           Data.Bits
import           Data.ByteString.Lazy       (ByteString, hGet, isSuffixOf)
...
loadClassFromJar clNm jr = do
  ...
  case M.lookup k (unJR jr) of
    ...
    Just (jarFn, dent) ->
      withBinaryFile jarFn ReadMode $ \h -> do
      ...
      -- the variable-size fields (filename and "extra")
        hSeek h RelativeSeek $ fromIntegral $ 26 + dentFilenameLen dent + dentXtraFldLen dent
        let uncompSz = fromIntegral $ dentUncompSz dent
            compSz = fromIntegral $ dentCompSz dent
        bytes <-
          case dentCompMethod dent of
            0x0 -> hGet h uncompSz
...
addJar jr fn = do
  ...
  let parseBytes n p = runGet p <$> hGet h (fromIntegral n)
      seekFromEnd = hSeek h SeekFromEnd . negate . fromIntegral {- Grr! -}
...
dirEnd = do
  ...
  DirEnd <$> getWord16le <*> getWord16le <*> getWord16le <*> getWord16le <*> getWord32le <*> getWord32le <*> do
    clen <- getWord16le
    if clen > 0
